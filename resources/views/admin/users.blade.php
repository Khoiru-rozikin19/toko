...
<div class="space-y-6 sm:space-y-8">
    ...
    <div class="border-b border-slate-200 dark:border-slate-800 pb-5">
        <h2 class="text-2xl sm:text-3xl font-extrabold text-slate-850 dark:text-slate-100 tracking-tight">Manajemen Pengguna</h2>
        <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Kelola persetujuan verifikasi akun baru, permohonan upgrade Seller, serta kelola peran dan status akun seluruh pengguna.</p>
    </div>
        ...
        <div id="tab-all-users" class="tab-content hidden transition-all duration-300">
            @if($allUsers->isEmpty())
                <div class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl p-12 text-center shadow-sm">
                    ...
                    <p class="text-sm text-slate-400 dark:text-slate-550 mt-1">Belum ada akun pengguna lain terdaftar di sistem.</p>
                </div>
            @else
                <div class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl overflow-hidden shadow-sm">
                    <div class="overflow-x-auto">
                            ...
                            <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                                @foreach($allUsers as $user)
                                    <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-850/30 transition-colors">
                                        ...
                                        <td class="px-6 py-4">
                                            <div class="font-bold text-slate-800 dark:text-slate-100">{{ $user->name }}</div>
                                            <div class="text-xs text-slate-550 dark:text-slate-400">{{ $user->email }}</div>
                                        ...
                                        <td class="px-6 py-4">
                                            <form action="{{ url('/admin/users/'.$user->id.'/update-role') }}" method="POST" class="flex items-center space-x-2">
                                                ...
                                                <button type="submit" class="px-3 py-1.5 bg-blue-600 hover:bg-blue-750 text-white rounded-xl text-[10px] font-bold active:scale-95 transition-all duration-200">
                                                    Ubah
                                                </button>
                                            </form>
                                        </td>
                                        ...
                                        <td class="px-6 py-4">
                                            @if($user->is_verified)
                                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-emerald-100 dark:bg-emerald-950/30 text-emerald-800 dark:text-emerald-450 border border-emerald-200/10">
                                                    Aktif
                                                </span>
                                            @else
                                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-rose-100 dark:bg-rose-950/30 text-rose-800 dark:text-rose-450 border border-rose-200/10">
                                                    Suspended
                                                </span>
                                            @endif
                                        </td>
                                        ...
                                        <td class="px-6 py-4 text-right">
                                            <div class="flex justify-end space-x-2">
                                                <form action="{{ url('/admin/users/'.$user->id.'/toggle-status') }}" method="POST">
                                                    @csrf
                                                    @if($user->is_verified)
                                                        <button type="submit" class="px-3 py-1.5 bg-amber-50 dark:bg-amber-950/20 border border-amber-200/20 text-amber-700 dark:text-amber-450 rounded-xl text-xs font-bold active:scale-95 transition-all duration-200">
                                                            Suspend
                                                        </button>
                                                    @else
                                                ...
                                                <form action="{{ url('/admin/users/'.$user->id.'/delete') }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus akun {{ $user->name }} secara permanen?')">
                                                    @csrf
                                                    <button type="submit" class="px-3 py-1.5 bg-rose-50 dark:bg-rose-950/20 border border-rose-100 dark:border-rose-900/30 text-rose-600 dark:text-rose-450 rounded-xl text-xs font-bold active:scale-95 transition-all duration-200">
                                                        Hapus
                                                    </button>
                                                </form>
                                            </div>
